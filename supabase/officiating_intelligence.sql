...
options_json as (
  select jsonb_build_object(
    'opponents', coalesce((
      ...
      from (
        select distinct opponent as value
        from context_rows
        where coalesce(opponent, '') <> ''
      ) options
    ), '[]'::jsonb),
    'homeRoad', coalesce((
      select jsonb_agg(value order by value)
      from (
        select distinct home_road as value
        from context_rows
        where coalesce(home_road, '') <> ''
      ) options
    ), '[]'::jsonb),
    'crewChiefs', coalesce((
      select jsonb_agg(value order by value)
      from (
        select distinct crew_chief_name as value
        from context_rows
        where coalesce(crew_chief_name, '') <> ''
      ) options
    ), '[]'::jsonb),
    'whistlingOfficials', coalesce((
      select jsonb_agg(value order by value)
      from (
        select distinct whistling_official_name as value
        from context_rows
        where coalesce(whistling_official_name, '') <> ''
      ) options
    ), '[]'::jsonb),
    'infractionTypes', coalesce((
      select jsonb_agg(value order by value)
      from (
        select distinct infraction_type_name as value
        from context_rows
        where coalesce(infraction_type_name, '') <> ''
      ) options
    ), '[]'::jsonb)
