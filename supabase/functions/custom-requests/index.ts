...
type PromiseCache<T> = {
  ...
  set(key: string, value: Promise<T>): PromiseCache<T>;
  delete(key: string): boolean;
};

function createPromiseCache<T>(maxEntries: number, ttlMs: number): PromiseCache<T> {
  ...
  const cache: PromiseCache<T> = {
    ...
    set(key: string, value: Promise<T>) {
      ...
      trackedPromise = value.catch((error) => {
        const current = entries.get(key);
        if (current?.promise === trackedPromise) entries.delete(key);
        throw error;
      });
...
const SEASON_GAMES_CACHE = createPromiseCache<Record<string, unknown>[]>(4, 30 * MINUTE_MS);
const GAME_DETAILS_CACHE = createPromiseCache<Record<string, unknown>>(900, 20 * MINUTE_MS);
const TEAM_SEASON_DATASET_CACHE = createPromiseCache<{
  rows: CachedTeamGameRow[];
  skippedGames: Array<{ gameId: string; gameDate: string; error: string }>;
}>(48, 20 * MINUTE_MS);
const PLAYER_SEASON_DATASET_CACHE = createPromiseCache<{
  rows: CachedPlayerGameRow[];
  skippedGames: Array<{ gameId: string; gameDate: string; error: string }>;
}>(48, 20 * MINUTE_MS);
const PLAYER_PERIOD_POINTS_DATASET_CACHE = createPromiseCache<{
  rows: CachedPlayerGameRow[];
  skippedGames: Array<{ gameId: string; gameDate: string; error: string }>;
}>(32, 20 * MINUTE_MS);
const TEAM_PERIOD_POINTS_DATASET_CACHE = createPromiseCache<{
  rows: CachedTeamGameRow[];
  skippedGames: Array<{ gameId: string; gameDate: string; error: string }>;
}>(32, 20 * MINUTE_MS);
const TEAM_ROSTERS_CACHE = createPromiseCache<Array<{
  teamId: string;
  team: (typeof NBA_TEAMS)[number];
  players: Array<{ playerId: string; playerName: string }>;
}>>(4, 6 * 60 * MINUTE_MS);
const GAME_MINUTES_CACHE = createPromiseCache<Record<string, unknown>>(900, 20 * MINUTE_MS);
const TEAM_ON_OFF_DATASET_CACHE = createPromiseCache<{
  rows: CachedTeamGameRow[];
  skippedGames: Array<{ gameId: string; gameDate: string; error: string }>;
}>(32, 20 * MINUTE_MS);
const LEAGUE_TEAM_DATASET_CACHE = createPromiseCache<{
  rows: CachedTeamGameRow[];
  skippedGames: Array<{ gameId: string; gameDate: string; error: string }>;
}>(4, 20 * MINUTE_MS);
const DEFAULT_SEASON_CACHE = createPromiseCache<string>(4, 10 * MINUTE_MS);

const ROSTER_SNAPSHOT_BY_TEAM = rosterSnapshotByTeam as Record<string, Array<Record<string, unknown>>>;
